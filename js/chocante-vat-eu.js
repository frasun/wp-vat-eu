...
(function ($) {
	...
	checkoutForm.on("change", `${companyField}, ${taxIdField}`, function () {
		...
		if (!companyFieldHasValue && taxIdFieldHasValue) {
			companyFieldParent
				.removeClass(VALIDATED_CLASS)
				.addClass(`${REQUIRED_CLASS} ${INVALID_CLASS}`);
		}

		if (!companyFieldHasValue && !taxIdFieldHasValue) {
			window.requestAnimationFrame(function () {
				companyFieldParent.removeClass(
					`${REQUIRED_CLASS} ${INVALID_CLASS} ${VALIDATED_CLASS}`,
				);
				taxIdFieldParent.removeClass(
					`${REQUIRED_CLASS} ${INVALID_CLASS} ${VALIDATED_CLASS}`,
				);
			});
		}
